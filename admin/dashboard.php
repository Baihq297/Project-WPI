...
<head>
  ...
  <style>
    /* (CSS Anda sudah benar, tidak saya ubah) */
    :root {
      --merah: #b22222;
      --kuning: #ffc107;
      --putih: #fffdfa;
    ...
    @media (max-width: 768px) {
      ...
      .table img { width: 50px !important; height: 50px !important; }
    }
    .image-preview {
        width: 120px;
        height: 120px;
  ...
  <div class="container d-flex justify-content-between align-items-center">
    ...
    <div class="d-flex align-items-center">
      ...
      <span class="text-light me-3">
        Halo, <strong class="text-warning"><?= htmlspecialchars($_SESSION['pelanggan']['nama']) ?></strong>
      </span>
...
<div class="container my-4">
    ...
    <div class="col-md-4">
      <div class="card text-center">
        <div class="card-body">
          ...
          <h6 class="mt-2 mb-1 text-secondary">Total Menu</h6>
          <h3><?= $totalMenu ?></h3>
        </div>
      </div>
    </div>
    <div class="col-md-4">
      <div class="card text-center">
        <div class="card-body">
          ...
          <h6 class="mt-2 mb-1 text-secondary">Total Pesanan</h6>
          <h3><?= $totalPesanan ?></h3>
        </div>
      </div>
    </div>
    <div class="col-md-4">
      <div class="card text-center">
        <div class="card-body">
          ...
          <h6 class="mt-2 mb-1 text-secondary">Total Pelanggan</h6>
  ...
  <div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div>
      ...
      <button type="button" class="btn btn-warning shadow-sm" data-bs-toggle="modal" data-bs-target="#tambahMenuModal">
  ...
  <div class="table-responsive shadow-sm rounded mb-5">
    <table class="table align-middle">
      <thead class="text-center">
        <tr>
          <th>No</th> <th>Gambar</th> <th>Nama Menu</th> <th>Harga</th> <th>Stok</th> <th>Status</th> <th>Aksi</th>
        </tr>
      </thead>
      <tbody>
        <?php
              ...
              <tr>
                ...
                <td class='text-center'>
                  <div class='form-check form-switch d-flex justify-content-center'>
                    <input type='checkbox' class='form-check-input toggle-status' data-id='<?= $r['id_menu'] ?>' <?= $checked ?>>
                  </div>
                </td>
                <td class='text-center'>
                  <button type='button' class='btn btn-sm btn-primary btn-edit-menu' data-bs-toggle='modal' data-bs-target='#editMenuModal' data-id='<?= $r['id_menu'] ?>'>
        ...
        <?php 
            ...
            echo "<tr><td colspan='7' class='text-center text-muted p-4'>Belum ada data makanan.</td></tr>";
        }
        ?>
      </tbody>
    </table>
    ...
    <table class="table align-middle">
      ...
      <tbody>
        ...
        <?php 
            ...
            echo "<tr><td colspan='7' class='text-center text-muted p-4'>Belum ada data minuman.</td></tr>";
        }
        ?>
      </tbody>
    </table>
  ...
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content" style="border-radius: 16px;">
      <div class="modal-header">
        <h5 class="modal-title fw-bold text-danger" id="tambahMenuModalLabel"><i class="bi bi-plus-circle"></i> Tambah Menu Baru</h5>
        ...
        <form id="addMenuForm" method="post" enctype="multipart/form-data">
          <div class="row mb-3">
            <div class="col-md-6">
              ...
              <input type="text" name="nama_menu" class="form-control" placeholder="Contoh: Ayam Geprek" required>
            </div>
            <div class="col-md-6">
          ...
          <div class="row mb-3">
            <div class="col-md-6">
              ...
              <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" name="status" id="statusSwitchModal" value="tersedia" checked>
                <label class="form-check-label" for="statusSwitchModal" id="statusLabelModal">Tersedia</label>
              </div>
            </div>
            <div class="col-md-6">
              <label class="form-label">Gambar Menu</label>
              <div class="d-flex align-items-start gap-3">
                  <img src="" id="imagePreview" class="image-preview rounded shadow-sm">
                  <div>
                      <input type="file" name="gambar" id="fileInputModal" class="file-input-modal" accept="image/png, image/jpeg">
                      <label for="fileInputModal" class="btn btn-outline-danger file-upload-label">
                          <i class="bi bi-upload"></i> Pilih Gambar
                      </label>
                      <div class="form-text mt-1" id="fileName">No file chosen</div>
                  </div>
              </div>
            </div>
          </div>
      ...
      <div class="modal-footer">
        ...
        <button type="submit" form="addMenuForm" class="btn btn-danger" id="saveMenuButton">
...
<div class="modal fade" id="editMenuModal" tabindex="-1" aria-labelledby="editMenuModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content" style="border-radius: 16px;">
      <div class="modal-header">
        <h5 class="modal-title fw-bold text-danger" id="editMenuModalLabel"><i class="bi bi-pencil-square"></i> Edit Menu</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <form id="editMenuForm" method="post" enctype="multipart/form-data">
          ...
          <div class="row mb-3">
            <div class="col-md-6">
              <label class="form-label">Status Menu</label>
              <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" name="status" id="edit_statusSwitch" value="tersedia">
                <label class="form-check-label" for="edit_statusSwitch" id="edit_statusLabel">Tersedia</label>
              </div>
            </div>
            <div class="col-md-6">
              <label class="form-label">Gambar Menu</label>
              <div class="d-flex align-items-start gap-3">
                  <img src="" id="edit_imagePreview" class="image-preview rounded shadow-sm">
                  <div>
                      <input type="file" name="gambar" id="edit_fileInput" class="file-input-modal" accept="image/png, image/jpeg">
                      <label for="edit_fileInput" class="btn btn-outline-danger file-upload-label">
                          <i class="bi bi-upload"></i> Ganti Gambar
                      </label>
                      <div class="form-text mt-1" id="edit_fileName">Ganti jika perlu...</div>
                  </div>
              </div>
            </div>
          </div>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
        <button type="submit" form="editMenuForm" class="btn btn-danger" id="updateMenuButton">
...
<script>
// (Fungsi chart dan showToast tidak berubah)
...
$(document).ready(function() {
  // (Notifikasi Session dan Delete tidak berubah)
  <?php if ($notif_msg): ?>
  ...
  // === PERBAIKAN LOGIKA TOGGLE STATUS ===
  $('.toggle-status').change(function() {
    let id = $(this).data('id');
    let status = $(this).is(':checked') ? 'aktif' : 'nonaktif';
    let currentToggle = $(this); // Simpan toggle yang diklik
    ...
    $.post('../api/update_status.php', { id_menu: id, status: status }, function(res) {
      if (res.success) {
        ...
        // Paksa toggle kembali ke posisi 'off'
  ...
  // --- Modal Tambah Menu (Logika Anda sudah benar) ---
  const addMenuModal = new bootstrap.Modal(document.getElementById('tambahMenuModal'));
  ...
  if (statusSwitchModal && statusLabelModal) {
      statusSwitchModal.addEventListener('change', function() {
          statusLabelModal.textContent = this.checked ? 'Tersedia' : 'Habis';
      });
  }
  const fileInputModal = document.getElementById('fileInputModal');
  const imagePreview = document.getElementById('imagePreview');
  const fileNameDisplay = document.getElementById('fileName');
  if (fileInputModal && imagePreview && fileNameDisplay) {
      ...
      $.ajax({
          ...
          success: function(res) {
              if (res.success) {
                  ...
                  Swal.fire({ title: 'Oops, Gagal!', html: res.message || 'Gagal menyimpan data.', icon: 'error', confirmButtonColor: '#b22222' });
              }
          },
          error: function() {
          ...
          complete: function() {
              saveMenuButton.prop('disabled', false).text('Simpan Menu');
          }
      });
  });
  
  $('#tambahMenuModal').on('hidden.bs.modal', function () {
      addMenuForm[0].reset();
      imagePreview.style.display = 'none';
      fileNameDisplay.textContent = 'No file chosen';
      statusLabelModal.textContent = 'Tersedia';
  });


  // --- Logika BARU untuk Modal Edit Menu ---
  ...
  if (editFileInput && editImagePreview && editFileName) {
      editFileInput.addEventListener('change', function(event) {
          ...
          if (file) {
              ...
              editFileName.textContent = 'Ganti jika perlu...';
          }
      });
  }

  // Toggle di Modal Edit
  const editStatusSwitch = document.getElementById('edit_statusSwitch');
  const editStatusLabel = document.getElementById('edit_statusLabel');
  if (editStatusSwitch && editStatusLabel) {
      editStatusSwitch.addEventListener('change', function() {
          editStatusLabel.textContent = this.checked ? 'Tersedia' : 'Habis';
      });
  }

  // 1. Saat tombol pensil (edit) diklik -> Ambil data
  $('.btn-edit-menu').on('click', function() {
      const id_menu = $(this).data('id');
      
      editMenuForm[0].reset();
      $('#editMenuModalLabel').text('Memuat data menu...');
      $('#edit_imagePreview').attr('src', '').hide();
      ...
      $.ajax({
          ...
          success: function(res) {
              if (res.success) {
                  const data = res.data;
                  $('#editMenuModalLabel').text('Edit Menu: ' + data.nama_menu);
                  ...
                  // === PERBAIKAN LOGIKA TOGGLE EDIT ===
                  // Logika di dashboard (is_tersedia && stok > 0) harus sama di sini
                  const isTersedia = data.is_tersedia;
                  const isStokAda = data.stok > 0;
                  const shouldBeChecked = (isTersedia && isStokAda);

                  $('#edit_statusSwitch').prop('checked', shouldBeChecked);
                  $('#edit_statusLabel').text(shouldBeChecked ? 'Tersedia' : 'Habis');
                  // ================================
                  ...
                  Swal.fire('Gagal', res.message || 'Gagal mengambil data.', 'error');
                  editMenuModal.hide();
              }
          },
          error: function() {
  ...
  // 2. Saat form edit disubmit -> Simpan data
  editMenuForm.on('submit', function(e) {
      ...
      $.ajax({
          ...
          complete: function() {
              updateMenuButton.prop('disabled', false).text('Simpan Perubahan');
          }
      });
  });
