...
require_once "../includes/mail_config.php"; // Gunakan konfigurasi pengiriman email global
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($email === '') {
    ...
    if ($row = mysqli_fetch_assoc($result)) {
      // (Semua logika PHP Anda untuk kirim email sudah benar)
      // ... (membuat token, update DB, kirim email) ...
...
<head>
  ...
  <style>
    ...
    .header img { width:90px; border-radius:8px; margin-bottom:10px; }
    .content { padding:25px; text-align:center; }
    .content h2 { color:#333; margin-bottom:10px; }
    .content p { line-height:1.6; color:#555; margin:0 0 20px 0; }
...
<body>
  <div class="container">
    <div class="header">
      <img src="cid:logo_gc" alt="Logo Goreng Chicken">
...
HTML;
      $send = sendMail($email, 'Reset Password Akun Anda', $body);
...
<head>
  <meta charset="UTF-8">
  <title>Lupa Password (Profil) - Goreng Chicken</title>
  ...
  <style>
    ...
    .alert { font-size: 14px; padding: 10px 15px; margin-bottom: 15px; border-radius: 8px; }
  </style>
</head>
<body>
...
<div class="reset-box">
  ...
  <div class="reset-body">
    ...
    <form method="POST">
      <div class="mb-3">
        <input type="email" name="email" value="<?= isset($email) ? htmlspecialchars($email) : '' ?>" class="form-control" placeholder="Masukkan Email Anda" required>
      </div>
    ...
    <div class="mt-4 text-center">
      <a href="../pages/profile.php" class="text-decoration-none">
        <i class="bi bi-arrow-left-square"></i> Kembali ke Profil Saya
      </a>
    </div>
  </div>
</div>
